...
namespace SagaGateway
{
    public class GatewayClient : SagaLib.Client
    {
        ...
        {
            if (p.ServerID == 0x0502)
            {
                Logger.ShowInfo("CTGA_RECONNECTLOGIN (0x0502) received for session " + p.SessionID, null);
            ...
            if (p.data.Length < 14)
            {
                Logger.ShowWarning(string.Format("Short packet (size={0}, ServerID:{1:X4}) from session {2}",
                    p.data.Length, p.ServerID, p.SessionID), null);
                return;
            }
        ...
        /// Logs out the map session (saves character, removes from world),
        /// sends AGCT_RECONNECT success, then requests the character list.
        /// </summary>
        private void OnReturnToCharacterSelect()
        {
            Logger.ShowInfo("Return to character select for session " + this.SessionID, null);
            try
            {
                if (this.state == SESSION_STATE.MAP && this.MapSession != null)
                {
        ...
        /// <summary>
        /// Constructs a CM_CHARLIST request and forwards it to the login
        ...
        private void SendCharacterListRequest()
        {
            try
            {
                byte[] charListData = new byte[] { 0x04, 0x00, 0x01, 0x06 };
                Gateway.Login.SendToLogin(charListData, this.SessionID);
            }
            catch (Exception ex)
            {
                Logger.ShowError(ex, null);
